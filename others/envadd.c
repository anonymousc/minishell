...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/12 22:39:31 by aait-bou          #+#    #+#             */
/*   Updated: 2024/12/12 22:39:32 by aait-bou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_env	*make_env(char **env_original)
{
	...
	while (env_original && env_original[i])
	{
		...
		if (!delimiter)
			return (NULL);
		add_back(&envir, lstnew_env(ft_strndup(env_original[i], delimiter
					- env_original[i]), ft_strdup(delimiter + 1)));
		i++;
	}
	return (envir);
}
